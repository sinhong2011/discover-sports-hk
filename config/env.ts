/**
 * Environment configuration for the App Name app
...
export function createAppConfig(): AppConfig {
  try {
    const workerApiConfig: ApiConfig = {
      baseUrl: getEnvVar(
        'WORKER_API_BASE_URL',
        'https://openpandata-worker.openpandata.workers.dev'
      ),
      ...
      bundleId: getOptionalEnvVar('BUNDLE_ID', 'com.openpandata.discoversportshk'),
    };
...
export function getConfig(): AppConfig {
  if (isDevelopment()) {
    try {
      ...
      console.warn('Using development config due to missing environment variables:', error);
